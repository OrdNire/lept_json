    ...
    switch (src->type) {
        ...
        case LEPT_OBJECT:
            ...
            for (i = 0; i < src->u.o.size; i++) {
                ...
                // dst 刚分配内存，不需要释放 k，会导致野指针free出错
        ...
        case LEPT_OBJECT:
            ...
            for (i = 0; i < lhs->u.o.size; i++) {
                ...
                if ((index = lept_find_object_index(rhs, lhs->u.o.m[i].k, lhs->u.o.m[i].klen)) == LEPT_KEY_NOT_EXIST)
                    return 0;
                if (!lept_is_equal(&lhs->u.o.m[i].v, lept_get_object_value(rhs, index)))
                    return 0;
            }
            return 1;
        default:
...
void lept_erase_array_element(lept_value* v, size_t index, size_t count) {
    ...
    while (i < v->u.a.size) lept_free(&v->u.a.e[i ++]);
    v->u.a.size -= count;
}

#define FREE_LEPT_MEMBER(m)\
    do {\
        free((m)->k);\
        (m)->k = NULL;\
        (m)->klen = 0;\
...
lept_value* lept_set_object_value(lept_value* v, const char* key, size_t klen) {
    assert(v != NULL && v->type == LEPT_OBJECT && key != NULL);
    lept_value* t = lept_find_object_value(v, key, klen);
    if (t != NULL) return t;    // 存在key
    else {  // 不存在key
        if (v->u.o.capacity == v->u.o.size)
...
void lept_swap_member(lept_member* lhs, lept_member* rhs) {
    assert(lhs != NULL && rhs != NULL);
    if (lhs != rhs) {
        lept_member temp;
